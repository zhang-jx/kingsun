        ...
        synchronized (mLock) {
            ...
            if (index >= 0) {
                if (mDyingViews.contains(view)) {
                    ...
                    //*/ freeme wanbao.zhang,2017.0729,Use float window manager.
                    boolean isBlackList = false;
                    try {
                        if (getWindowManagerService().getBlackListFloat().containsKey(wparams.packageName)) {
                            isBlackList = true;
                        }
                        android.util.Log.d("SC_FloatWindowManager", "isBlackList : " + isBlackList + "," + wparams.packageName);
                    } catch (Exception err){
                        android.util.Log.d("SC_FloatWindowManager","err:"+err.toString());
                    }
                    if (isBlackList) {
                        return;
                    } else {
